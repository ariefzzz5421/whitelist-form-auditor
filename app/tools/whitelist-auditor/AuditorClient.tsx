...
export default function AuditorClient() {
  ...
  async function runAudit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.ok || !response.body) throw new Error(await readAuditError(response));
...
async function readAuditError(response: Response) {
  const status = `HTTP ${response.status}`;
  const contentType = response.headers.get("content-type") || "";

  if (contentType.includes("application/json")) {
    const payload = await response.json().catch(() => null);
    return payload?.error ? `${status}: ${payload.error}` : `${status}: Audit failed.`;
  }

  if (contentType.includes("text/html")) {
    return `${status}: The audit backend returned an HTML error page instead of a JSON/SSE response.`;
  }

  const text = await response.text().catch(() => "");
  return text ? `${status}: ${text.slice(0, 240)}` : `${status}: Audit failed.`;
}
